...
colindex<-c('IP','ENEG','NDE','ZIF','POLY','CTUBE','BIO','MT','NC','MNN','PYRID','PYRRO','RAMAN','BET','PTEMP',	'PTIME','RR','EC','CL','NMT','CPGC','EPH','ovpCL')
...
data_PC_train=read.csv("train_OVPMCL.csv",head=T)
data_PC_test=read.csv("test_OVPMCL.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(2)
forest_huge <- randomForest(ovpCL ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
plot(forest_huge, main = "Learning curve of the forest")
??randomForest
forest <- randomForest(ovpCL ~ .,type=classification, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-23],'ovpCL'),
                 type=c(rep('Electronic_param',3),rep('Morphology_Param',4),rep('Catal_prop',7),rep('Synthesis_param',3),rep('Test_param',5),rep('Target',1)),
                 color=c(rep('#98dbef',3),rep('#a4e192',4),rep('#ffc177',7),rep('#000000',3),rep('#808000',5),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-23],'ovpCL'))
...
x_y<-data.frame(Source=c(rep('ovpCL',22)),
...
##################################################
total_inter_counts<-c(229, 188, 130, 55, 46, 46, 60, 200, 158, 154, 210, 165, 187, 203, 188, 93, 177, 144, 194, 56, 62, 225)
...
red_index<-((EDG $ Weight> (nthmax(EDG$Weight,23)))&(EDG$Source!=22)&(EDG$Target!=22))[,1]
##################################################
