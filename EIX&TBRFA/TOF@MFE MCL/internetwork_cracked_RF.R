...
??randomForest
forest <- randomForest(TOFMCL ~ .,type=classification, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-22],'TOFMCL'),
                 ...
                 color=c(rep('#98dbef',3),rep('#a4e192',3),rep('#ffc177',7),rep('#000000',3),rep('#808000',5),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-22],'TOFMCL'))
...
x_y<-data.frame(Source=c(rep('TOFMCL',21)),
...
##################################################
total_inter_counts<-c(764, 825, 684, 241, 257, 235, 1279, 1092, 952, 803, 736, 834, 805, 1024, 640, 659, 181, 292, 928, 277, 698)
