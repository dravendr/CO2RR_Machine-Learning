...
##################################################
set.seed(6)
forest_huge <- randomForest(FE95 ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
plot(forest_huge, main = "Learning curve of the forest")
forest <- randomForest(FE95 ~ .,type=classification, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-21],'FE95'),
                 ...
                 color=c(rep('#98dbef',3),rep('#a4e192',4),rep('#ffc177',7),rep('#000000',3),rep('#808000',3),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-21],'FE95'))
...
x_y<-data.frame(Source=c(rep('FE95',20)),
...
##################################################
total_inter_counts<-c(6584, 7134, 5268, 2400, 2105, 1832, 2027, 8189, 7302, 7811, 7220, 7382, 6503, 7504, 6335, 5625, 5925, 2644, 7364, 6526)
...
##################################################
#write.csv(nodes,paste0('test/nodes----','FE95','.csv'),row.names=FALSE,fileEncoding='UTF-8')
#write.csv(edges,paste0('test/edges----','FE95','.csv'),row.names=FALSE,fileEncoding='UTF-8')
##################################################
#EDG<-read_csv('test/edges----FE95.csv')
#NDS<-read_csv('test/nodes----FE95.csv')
##################################################
EDG<-edges
NDS<-nodes
