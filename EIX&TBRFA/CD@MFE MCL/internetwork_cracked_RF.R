...
colindex<-c('IP',	'ENEG',	'NDE',	'ZIF','CTUBE','BIO',	'MT',	'NC',	'MNN',	'PYRID',	'PYRRO',	'RAMAN',	'BET',	'PTEMP',	'RR',	'FLOWCELL',	'EC',	'CL',	'EPH',	'CLCDMFE')
...
data_PC_train=read.csv("train_CDMCL.csv",head=T)
data_PC_test=read.csv("test_CDMCL.csv",head=T)
#set.seed(39)
...
##################################################
set.seed(2)
# <- randomForest(CLCDMFE ~ .,type=classification, data = data_PC_train, ntree=10000, importance = TRUE)
#plot(forest_huge, main = "Learning curve of the forest")
??randomForest
forest <- randomForest(CLCDMFE ~ .,type=classification, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-20],'CLCDMFE'),
                 type=c(rep('Electronic_param',3),rep('Morphology_Param',3),rep('Catal_prop',7),rep('Synthesis_param',2),rep('Test_param',4),rep('Target',1)),
                 color=c(rep('#98dbef',3),rep('#a4e192',3),rep('#ffc177',7),rep('#000000',2),rep('#808000',4),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-20],'CLCDMFE'))
...
x_y<-data.frame(Source=c(rep('CLCDMFE',19)),
...
##################################################
total_inter_counts<-c(2849, 3728, 3055, 1356, 1081, 1118, 4001, 3631, 3745, 3828, 3829, 3526, 3612, 3172, 3147, 2685, 1379, 3675, 3929)
...
red_index<-((EDG $ Weight> (nthmax(EDG$Weight,20)))&(EDG$Source!=19)&(EDG$Target!=19))[,1]
##################################################
