...
##################################################
set.seed(2)
forest_huge <- randomForest(CD ~ .,type=regression, data = data_PC_train, ntree=10000, importance = TRUE)
plot(forest_huge, main = "Learning curve of the forest")
??randomForest
forest <- randomForest(CD ~ .,type=regression, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-20],'CD'),
                 ...
                 color=c(rep('#98dbef',3),rep('#a4e192',3),rep('#ffc177',7),rep('#000000',2),rep('#808000',4),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-20],'CD'))
...
##################################################
total_inter_counts<-c(2764, 3769, 2869, 2127, 1036, 1059, 3995, 3642, 3555, 3669, 4251, 3612, 3786, 3113, 2961, 3060, 2826, 3717, 4243)
