...
colindex<-c('IP','ENEG','NDE','ZIF','POLY','CTUBE','BIO','MT','NC','MNN','PYRID','PYRRO','RAMAN','BET','PTEMP',	'PTIME','RR','EC','CL','NMT','CPGC','EPH','ovp')
...
##################################################
set.seed(2)
forest_huge <- randomForest(ovp ~ .,type=regression, data = data_PC_train, ntree=10000, importance = TRUE)
plot(forest_huge, main = "Learning curve of the forest")
??randomForest
forest <- randomForest(ovp ~ .,type=regression, data = data_PC_train, ntree=num_trees, importance = TRUE)
...
##################################################
type<-data.frame(label=c(colindex[-23],'ovp'),
                 ...
                 color=c(rep('#98dbef',3),rep('#a4e192',4),rep('#ffc177',7),rep('#000000',3),rep('#808000',5),
                         rep('#ffb6d4',1)))
nodes<-data.frame(id=c(1:length(colindex)),label=c(colindex[-23],'ovp'))
...
x_y<-data.frame(Source=c(rep('ovp',22)),
...
##################################################
total_inter_counts<-c(983, 1024, 861, 322, 202, 231, 244, 880, 931, 910, 1013, 872, 1158, 986, 441, 952, 912, 875, 1042, 304, 319, 1030)
